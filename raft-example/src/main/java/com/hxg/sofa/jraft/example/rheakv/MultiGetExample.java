...
public class MultiGetExample {
    ...
    public static void multiGet(final RheaKVStore rheaKVStore) {
        ...
        for (int i = 0; i < 10; i++) {
            final byte[] bytes = BytesUtil.writeUtf8("multi_get_demo_" + i);
            keys.add(bytes);
        ...
        for (Map.Entry<ByteArray, byte[]> entry : f1.join().entrySet()) {
            LOG.info("Async multiGet: key={}, value={}", BytesUtil.readUtf8(entry.getKey().getBytes()),
        ...
        for (Map.Entry<ByteArray, byte[]> entry : map1.entrySet()) {
            LOG.info("Sync multiGet: key={}, value={}", BytesUtil.readUtf8(entry.getKey().getBytes()), BytesUtil.readUtf8(entry.getValue()));
        }
