...
public class ReverseScanExample {
    ...
    public static void scan(final RheaKVStore rheaKVStore) {
        ...
        for (int i = 0; i < 10; i++) {
            final byte[] bytes = BytesUtil.writeUtf8("scan_demo_" + i);
            keys.add(bytes);
        ...
        final String firstKeyString = BytesUtil.readUtf8(firstKey);
        final String lastKeyString = BytesUtil.readUtf8(lastKey);
        ...
        for (final CompletableFuture<List<KVEntry>> f : new CompletableFuture[] { f1, f2, f3, f4 }) {
            for (final KVEntry kv : f.join()) {
                LOG.info("Async reverseScan: key={}, value={}", BytesUtil.readUtf8(kv.getKey()), BytesUtil.readUtf8(kv.getValue()));
            }
        }
        ...
        for (final List<KVEntry> l : new List[] { l1, l2, l3, l4 }) {
            for (final KVEntry kv : l) {
                LOG.info("sync reverseScan: key={}, value={}", BytesUtil.readUtf8(kv.getKey()), BytesUtil.readUtf8(kv.getValue()));
            }
        }
    }
}
