...
import com.hxg.sofa.jraft.error.InvokeTimeoutException;
...
public class AbstractClientServiceTest {
    ...
    public void testCancel() throws Exception {
        ...
        RpcRequests.PingRequest request = TestUtils.createPingRequest();

        MockRpcResponseClosure<RpcRequests.ErrorResponse> done = new MockRpcResponseClosure<>();
        ...
        future.cancel(true);
        RpcRequests.ErrorResponse response = (RpcRequests.ErrorResponse) this.rpcResponseFactory.newResponse(null, Status.OK());
        cb.complete(response, null);

        // The closure should be notified with ECANCELED error code.
        done.latch.await();
        assertNotNull(done.status);
        Assert.assertEquals(RaftError.ECANCELED.getNumber(), done.status.getCode());
    }
    ...
    public void testInvokeWithDOneOnErrorResponse() throws Exception {
        ...
        assertTrue(msg instanceof RpcRequests.ErrorResponse);
        Assert.assertEquals(((RpcRequests.ErrorResponse) msg).getErrorMsg(), "failed");
