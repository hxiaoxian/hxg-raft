 ...
 */
package com.hxg.sofa.jraft.conf;
...
public class ConfigurationManagerTest {
    ...
    public void testTruncate() {
        ...
        Assert.assertEquals(9, this.confManager.getLastConfiguration().getId().getIndex());
        Assert.assertEquals(5, this.confManager.get(5).getId().getIndex());
        Assert.assertEquals(6, this.confManager.get(6).getId().getIndex());
        this.confManager.truncatePrefix(6);
        //truncated, so is snapshot index
        Assert.assertEquals(0, this.confManager.get(5).getId().getIndex());
        Assert.assertEquals(6, this.confManager.get(6).getId().getIndex());
        Assert.assertEquals(9, this.confManager.getLastConfiguration().getId().getIndex());
        ...
        Assert.assertEquals(0, this.confManager.get(5).getId().getIndex());
        Assert.assertEquals(6, this.confManager.get(6).getId().getIndex());
        Assert.assertEquals(7, this.confManager.getLastConfiguration().getId().getIndex());
        Assert.assertEquals(7, this.confManager.get(9).getId().getIndex());
    }
}
