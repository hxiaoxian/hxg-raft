...
import com.hxg.sofa.jraft.StateMachine;
import com.hxg.sofa.jraft.Status;
import com.hxg.sofa.jraft.entity.*;
...
import com.hxg.sofa.jraft.closure.LoadSnapshotClosure;
import com.hxg.sofa.jraft.closure.SaveSnapshotClosure;
...
public class FSMCallerTest {
    ...
    public void testOnCommittedError() throws Exception {
        ...
        Assert.assertEquals("Fail to get entry at index=11 while committed_index=11", this.fsmCaller.getError().getStatus()
            .getErrorMsg());
    }
    ...
    public void testOnSnapshotLoad() throws Exception {
        ...
        final RaftOutter.SnapshotMeta meta = RaftOutter.SnapshotMeta.newBuilder().setLastIncludedIndex(12).setLastIncludedTerm(1).build();
    ...
    public void testOnError() throws Exception {
        ...
        this.fsmCaller.flush();
        assertFalse(this.fsmCaller.getError().getStatus().isOk());
        Assert.assertEquals(EnumOutter.ErrorType.ERROR_TYPE_LOG, this.fsmCaller.getError().getType());
    ...
    public void testOnSnapshotLoadStale() throws Exception {
        ...
        final RaftOutter.SnapshotMeta meta = RaftOutter.SnapshotMeta.newBuilder().setLastIncludedIndex(5).setLastIncludedTerm(1).build();
        Mockito.when(reader.load()).thenReturn(meta);
        ...
        this.fsmCaller.onSnapshotLoad(new LoadSnapshotClosure() {
            ...
            public void run(final Status status) {
                assertFalse(status.isOk());
                Assert.assertEquals(RaftError.ESTALE, status.getRaftError());
                latch.countDown();
            }
