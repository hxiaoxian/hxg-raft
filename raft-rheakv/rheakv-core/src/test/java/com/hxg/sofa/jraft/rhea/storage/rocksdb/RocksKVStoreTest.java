 ...
 */
package com.hxg.sofa.jraft.rhea.storage.rocksdb;
...
import com.hxg.sofa.jraft.rhea.storage.KVStoreAccessHelper;
import com.hxg.sofa.jraft.rhea.storage.LocalLock;
import com.hxg.sofa.jraft.rhea.storage.TestSnapshotWriter;
...
import com.hxg.sofa.jraft.rhea.StoreEngineHelper;
...
import com.hxg.sofa.jraft.rhea.storage.KVIterator;
import com.hxg.sofa.jraft.rhea.storage.KVOperation;
import com.hxg.sofa.jraft.rhea.storage.KVState;
import com.hxg.sofa.jraft.rhea.storage.KVStateOutputList;
import com.hxg.sofa.jraft.rhea.storage.KVStoreClosure;
import com.hxg.sofa.jraft.rhea.storage.KVStoreSnapshotFile;
import com.hxg.sofa.jraft.rhea.storage.KVStoreSnapshotFileFactory;
import com.hxg.sofa.jraft.rhea.storage.RawKVStore;
import com.hxg.sofa.jraft.rhea.storage.RocksRawKVStore;
import com.hxg.sofa.jraft.rhea.storage.Sequence;
import com.hxg.sofa.jraft.rhea.storage.SstColumnFamily;
import com.hxg.sofa.jraft.rhea.storage.SyncKVStore;
import com.hxg.sofa.jraft.rhea.storage.TestClosure;
import com.hxg.sofa.jraft.rhea.storage.TestSnapshotReader;
...
public class RocksKVStoreTest extends BaseKVStoreTest {
    ...
    @Test
    public void getTest() {
        final byte[] key = KeyValueTool.makeKey("get_test");
    ...
    public void multiGetTest() {
        ...
        for (int i = 0; i < 10; i++) {
            byte[] key = KeyValueTool.makeKey("multi_test_key_" + i);
            byte[] value = KeyValueTool.makeValue("multi_test_value_" + i);
            keyList.add(key);
            valueList.add(value);
            this.kvStore.put(key, value, null);
        }
    ...
    public void getLocalIteratorTest() {
        ...
        final List<byte[]> valueList = Lists.newArrayList();
        for (int i = 0; i < 10; i++) {
            byte[] key = KeyValueTool.makeKey("iterator_test_key_" + i);
    ...
    public void containsKeyTest() {
        final byte[] key = KeyValueTool.makeKey("contains_key_test");
        ...
        final byte[] value = KeyValueTool.makeValue("contains_key_test_value");
        this.kvStore.put(key, value, null);
    ...
    public void scanTest() {
        ...
        List<KVEntry> entries = new SyncKVStore<List<KVEntry>>() {
            @Override
            public void execute(RawKVStore kvStore, KVStoreClosure closure) {
                kvStore.scan(KeyValueTool.makeKey("scan_test_key_"), KeyValueTool.makeKey("scan_test_key_" + 99), closure);
            }
        }.apply(this.kvStore);
        assertEquals(entries.size(), keyList.size());
        for (int i = 0; i < keyList.size(); i++) {
    ...
    public void reverseScanTest() {
        ...
        List<KVEntry> entries = new SyncKVStore<List<KVEntry>>() {
            @Override
            public void execute(RawKVStore kvStore, KVStoreClosure closure) {
                kvStore.reverseScan(KeyValueTool.makeKey("scan_test_key_" + 99), KeyValueTool.makeKey("scan_test_key_"), closure);
            }
        }.apply(this.kvStore);
        assertEquals(entries.size(), keyList.size());
        ...
        entries = new SyncKVStore<List<KVEntry>>() {
            @Override
            public void execute(RawKVStore kvStore, KVStoreClosure closure) {
                kvStore.reverseScan(KeyValueTool.makeKey("scan_test_key_" + 99), null, closure);
            }
        }.apply(this.kvStore);
        assertEquals(entries.size(), 20);
    ...
    public void getSequenceTest() throws InterruptedException {
        final byte[] seqKey = KeyValueTool.makeKey("seq_test");
    ...
    @Test
    public void putTest() {
        final byte[] key = KeyValueTool.makeKey("put_test");
    ...
    public void compareAndPutTest() {
        final byte[] key = KeyValueTool.makeKey("put_test");
        final byte[] value = KeyValueTool.makeValue("put_test_value");
        this.kvStore.put(key, value, null);

        final byte[] update = KeyValueTool.makeValue("put_test_update");
        KVStoreClosure kvStoreClosure = new BaseKVStoreClosure() {
            @Override
            public void run(Status status) {
                assertEquals(status, Status.OK());
    ...
    public void batchCompareAndPutTest() {
        ...
        for (int i = 1; i <= batchWriteSize; i++) {
            final byte[] key = KeyValueTool.makeKey("put_test" + i);
            final byte[] value = KeyValueTool.makeValue("put_test_value" + i);
        ...
        for (int i = 1; i <= batchWriteSize; i++) {
            final byte[] key = KeyValueTool.makeKey("put_test" + i);
            final byte[] value = KeyValueTool.makeValue("put_test_value" + i);
            final byte[] update = KeyValueTool.makeValue("put_test_update" + i);
            KVStoreClosure kvStoreClosure = new BaseKVStoreClosure() {
                @Override
                public void run(Status status) {
                    assertEquals(status, Status.OK());
    ...
    public void putListTest() {
        final List<KVEntry> entries = Lists.newArrayList();
        for (int i = 0; i < 10; i++) {
            entries.add(new KVEntry(KeyValueTool.makeKey("batch_put_test_key" + i), KeyValueTool.makeValue("batch_put_test_value" + i)));
        }
        this.kvStore.put(entries, null);
        final List<KVEntry> entries2 = new SyncKVStore<List<KVEntry>>() {
            @Override
            public void execute(RawKVStore kvStore, KVStoreClosure closure) {
                kvStore.scan(KeyValueTool.makeKey("batch_put_test_key"), KeyValueTool.makeKey("batch_put_test_key" + 99), closure);
            }
        }.apply(this.kvStore);
    ...
    public void putIfAbsentTest() {
        byte[] key = KeyValueTool.makeKey("put_if_absent_test");
    ...
    public void deleteTest() {
        final List<KVEntry> entries = Lists.newArrayList();
        for (int i = 0; i < 10; i++) {
            entries.add(new KVEntry(KeyValueTool.makeKey("del_test" + i), KeyValueTool.makeValue("del_test_value")));
        }
        this.kvStore.put(entries, null);
        this.kvStore.delete(KeyValueTool.makeKey("del_test5"), null);
        TestClosure closure = new TestClosure();
        this.kvStore.scan(KeyValueTool.makeKey("del_test"), KeyValueTool.makeKey("del_test" + 99), closure);
        List<KVEntry> entries2 = (List<KVEntry>) closure.getData();
        assertEquals(entries.size() - 1, entries2.size());
        closure = new TestClosure();
        this.kvStore.get(KeyValueTool.makeKey("del_test5"), closure);
        byte[] value = (byte[]) closure.getData();
        assertNull(value);
    }
    ...
    public void deleteRangeTest() {
        final List<KVEntry> entries = Lists.newArrayList();
        for (int i = 0; i < 10; i++) {
            entries.add(new KVEntry(KeyValueTool.makeKey("del_range_test" + i), KeyValueTool.makeValue("del_range_test" + i)));
        }
        this.kvStore.put(entries, null);
        // delete [del_range_test5, del_range_test8)
        this.kvStore.deleteRange(KeyValueTool.makeKey("del_range_test5"), KeyValueTool.makeKey("del_range_test8"), null);
        TestClosure closure = new TestClosure();
        this.kvStore.scan(KeyValueTool.makeKey("del_range_test"), KeyValueTool.makeKey("del_range_test" + 99), closure);
        final List<KVEntry> entries2 = (List<KVEntry>) closure.getData();
        assertEquals(entries.size() - 3, entries2.size());
        closure = new TestClosure();
        this.kvStore.get(KeyValueTool.makeKey("del_range_test5"), closure);
        byte[] value = (byte[]) closure.getData();
        assertNull(value);
        closure = new TestClosure();
        this.kvStore.get(KeyValueTool.makeKey("del_range_test6"), closure);
        value = (byte[]) closure.getData();
        assertNull(value);
        closure = new TestClosure();
        this.kvStore.get(KeyValueTool.makeKey("del_range_test7"), closure);
        value = (byte[]) closure.getData();
        assertNull(value);
        closure = new TestClosure();
        this.kvStore.get(KeyValueTool.makeKey("del_range_test8"), closure);
        value = (byte[]) closure.getData();
        assertNotNull(value);
    }
    ...
    public void deleteListTest() {
        ...
        for (int i = 0; i < 10; i++) {
            final byte[] key = KeyValueTool.makeKey("batch_del_test" + i);
            entries.add(new KVEntry(key, KeyValueTool.makeValue("batch_del_test_value")));
            keys.add(key);
        }
        this.kvStore.put(entries, null);
        this.kvStore.delete(keys, null);
        TestClosure closure = new TestClosure();
        this.kvStore.scan(KeyValueTool.makeKey("batch_del_test"), KeyValueTool.makeKey("batch_del_test" + 99), closure);
        List<KVEntry> entries2 = (List<KVEntry>) closure.getData();
    ...
    public void snapshotTest() throws Exception {
        ...
        for (int i = 0; i < 100000; i++) {
            ...
            this.kvStore.put(KeyValueTool.makeKey(v), KeyValueTool.makeValue(v), null);
        }
        ...
        assertNotNull(get(KeyValueTool.makeKey("1")));

        this.kvStore.put(KeyValueTool.makeKey("100001"), KeyValueTool.makeValue("100001"), null);
        assertNotNull(get(KeyValueTool.makeKey("100001")));
        ...
        assertNull(get(KeyValueTool.makeKey("1")));
        ...
        for (int i = 0; i < 100000; i++) {
            final String v = String.valueOf(i);
            Assert.assertArrayEquals(KeyValueTool.makeValue(v), get(KeyValueTool.makeKey(v)));
        }

        // key[100001] is put after the snapshot, so key[100001] should not exist.
        assertNull(get(KeyValueTool.makeKey("100001")));
    ...
    public void multiGroupSnapshotTest() throws Exception {
        ...
        regions.add(new Region(1, KeyValueTool.makeKey("0"), KeyValueTool.makeKey("1"), null, null));
        regions.add(new Region(2, KeyValueTool.makeKey("1"), KeyValueTool.makeKey("2"), null, null));
        regions.add(new Region(3, KeyValueTool.makeKey("2"), KeyValueTool.makeKey("3"), null, null));
        regions.add(new Region(4, KeyValueTool.makeKey("3"), KeyValueTool.makeKey("4"), null, null));
        regions.add(new Region(5, KeyValueTool.makeKey("4"), KeyValueTool.makeKey("5"), null, null));
        ...
        for (int i = 0; i < 5; i++) {
            this.kvStore.getSequence(KeyValueTool.makeKey(i + "_seq_test"), 10, null);
        }
        ...
        for (int i = 0; i < 4; i++) {
            final String v = String.valueOf(i);
            final byte[] seqKey = KeyValueTool.makeKey(i + "_seq_test");
            Assert.assertArrayEquals(KeyValueTool.makeValue(v), get(KeyValueTool.makeKey(v)));
        ...
        assertNull(get(KeyValueTool.makeKey("5")));
        final Sequence sequence = new SyncKVStore<Sequence>() {
            @Override
            public void execute(RawKVStore kvStore, KVStoreClosure closure) {
                kvStore.getSequence(KeyValueTool.makeKey("4_seq_test"), 10, closure);
            }
        }.apply(this.kvStore);
    ...
    public void getApproximateKeysInRangeTest() {
        ...
        for (int i = 0; i < 10000; i++) {
            entries.add(new KVEntry(KeyValueTool.makeKey("approximate_test" + i), KeyValueTool.makeValue("approximate_test_value")));
        }
        this.kvStore.put(entries, null);

        long approximateKeys = this.kvStore.getApproximateKeysInRange(KeyValueTool.makeKey("approximate_test" + 9999), null);
        assertEquals(1, approximateKeys);
        approximateKeys = this.kvStore.getApproximateKeysInRange(null, KeyValueTool.makeKey("approximate_test" + 9999));
        assertEquals(10000, approximateKeys);
        approximateKeys = this.kvStore.getApproximateKeysInRange(KeyValueTool.makeKey("approximate_test" + 9990),
            KeyValueTool.makeKey("approximate_test" + 9999));
    ...
    public void jumpOverTest() {
        ...
        this.kvStore.put(entries, null);
        final byte[] endKey = this.kvStore.jumpOver(KeyValueTool.makeKey("approximate_test0000"), 1000);
        final long approximateKeys = this.kvStore.getApproximateKeysInRange(KeyValueTool.makeKey("approximate_test0000"), endKey);
