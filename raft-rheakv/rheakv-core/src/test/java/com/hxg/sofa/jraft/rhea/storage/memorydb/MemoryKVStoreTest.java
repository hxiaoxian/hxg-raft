 ...
 */
package com.hxg.sofa.jraft.rhea.storage.memorydb;
...
import com.hxg.sofa.jraft.rhea.options.MemoryDBOptions;
...
import com.hxg.sofa.jraft.rhea.storage.MemoryRawKVStore;
...
public class MemoryKVStoreTest extends BaseKVStoreTest {
    ...
    public void reverseScanTest() {
        ...
        for (int i = 0; i < 10; i++) {
            byte[] key = KeyValueTool.makeKey("scan_test_key_" + i);
            byte[] value = KeyValueTool.makeValue("scan_test_key_" + i);
            keyList.add(key);
            valueList.add(value);
            this.kvStore.put(key, value, null);
        }
        for (int i = 0; i < 10; i++) {
            byte[] key = KeyValueTool.makeKey("no_scan_test_key_" + i);
            byte[] value = KeyValueTool.makeValue("no_scan_test_key_" + i);
            this.kvStore.put(key, value, null);
        }
    ...
    public void snapshotTest() throws Exception {
        ...
        for (int i = 0; i < 10000; i++) {
            this.kvStore.getSequence(KeyValueTool.makeKey((i % 100) + "seq_test"), 10, null);
        }
